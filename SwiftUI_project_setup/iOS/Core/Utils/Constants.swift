...
struct Constants{
    ...
    func getUniqueDeviceIdentifier() -> String {
        let uniqueIdKey = AppInfo.bundleId
        if let existingId = Constants.userDefaults.string(forKey: uniqueIdKey) {
            return existingId
        } else {
            let newId = Constants.uuid.uuidString
            Constants.userDefaults.set(newId, forKey: uniqueIdKey)
            return newId
        }
    }
